...
public final class BatchWriteBuffer implements WriteBuffer {
   ...
   public BatchWriteBuffer(final int maxBatchSize) {
      ...
      final int averageMessageSize = BytesUtils.nextPowOf2(BytesUtils.CACHE_LINE_LENGTH * 2);
      final int messageCount = (int) (BytesUtils.align(maxBatchSize, averageMessageSize) / averageMessageSize);
      final int requiredCapacity = RingBuffers.capacity(messageCount, averageMessageSize);
      ...
      this.commandExecutorAgent = new BatchWriteCommandExecutorAgent(writeRequests, new BatchAppender());
   }
   ...
   private void dutyCycleLoop() {
      ...
      while (!commandExecutorAgent.isClosed()) {
         if (commandExecutorAgent.doWork() > 0) {
            ...
            if (waiTimes < 100) {
               waiTimes++;
            } else if (waiTimes < 200) {
               Thread.yield();
               waiTimes++;
            } else if (waiTimes < 300) {
               LockSupport.parkNanos(1L);
               waiTimes++;
            } else if (waiTimes < 500) {
